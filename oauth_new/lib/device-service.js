		...
		for (const it of callbacks) {
			if (
				...
				try {
					...
					console.log(
						`Error updating state: "${error}" ${it.callbackUrls.stateCallback} ${it.access_token} ${it.username}`
					);
				}
			}
		}
		...
		for (const it of callbacks) {
			if (it.callbackAuth && it.callbackUrls) {
				try {
					...
					console.log(
						`Device added successfully ${it.callbackUrls.stateCallback}`
					);
				} catch (err) {
					console.log(
						`Error adding device: "${err}" ${it.callbackUrls.stateCallback} ${it.access_token}`
					);
				}
			}
		}
